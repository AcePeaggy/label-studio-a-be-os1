    ...
    def _is_deleted(self, instance):
        ...
        if organization_members := self.context.get("organization_members", None):
            ...
            organization_member_for_user = next(
                (organization_member for organization_member in organization_members if organization_member.user_id == instance.id),
                None,
            )
        else:
            ...
            organization_member_for_user = fast_first(OrganizationMember.objects.filter(user_id=instance.id, organization_id=org_id))
